...
def _preprocess(examples, tokenizer, context_column_name, target_column_name, context_max_length, target_max_length, prompt):
    ...
    # fill here
    ######
    inputs_texts = [prompt.format(context=c) for c in contexts]
    ...
    for lbl in label_ids:
        # PAD 토큰을 -100으로 바꿔서 loss에서 무시되도록 처리
        lbl_with_ignore = [(lid if lid != tokenizer.pad_token_id else -100) for lid in lbl]
        new_labels.append(lbl_with_ignore)
